...
//if yes, Navigator to Profile screen
  ...
  signUp(email, firstName, lastName, patronymic, phone, password, password2,
      datastore, birthDate) {
    String cardNum = "";
    var rnd = Random(9);
    for (int i = 1; i <= 6; ++i) {
...
// MOCKUP BEFORE THE ACTUAL DB IS CONNECTED
  createDataStoreMock() {
    ...
    BonusAccount bonusAccount = BonusAccount(0, [
      Purchase(11, 19, 2, 21, "Котелок", DateTime.now()),
      Purchase(3, 4, 22, 88, "Обдираловка", DateTime.now()),
      Purchase(7, 78, 12, 1, "Жаба", DateTime.now()),
      Purchase(2, 2, 9, 999, "Империя", DateTime.now())
    ]); //TODO прочтение из базы истории покупок
    // createDataStore
...
// Adds the new purchase to db
  void dbUpdate(DataStore datastore, int bonusSpent, int buySum, int level,
      String store, DateTime buyDateAndTime) {
    if (datastore.bonusAccount.currentBonuses < bonusSpent) {
      // If 'bonuses spent' random number is bigger than current bonuses in the account
    ...
    Purchase purchase =
        Purchase(buySum, bonusSpent, level, earnedBonus, store, buyDateAndTime);
