...
// Testing 3 levels of a loyalty program
    ...
    expect(
        datastore.bonusAccount.currentBonuses > 0 &&
            datastore.bonusAccount.currentBonuses < 100,
        true);
...
// final Map<String, String>
  ...
  test("SignUp method", () {
    ...
    DataStore datastore = controller.createDataStoreMock();
// final Map<String, String>
    controller.signUp("piotr@sich.ua", "Пётр", "Иванович", "Могила", "104224",
        "дубашуба", "дубашуба", datastore, birthDate);
    expect(datastore.myClient.phone, "104224");
    expect(datastore.myClient.firstName, "Пётр");
    expect(datastore.myClient.cardNum.length, 6);
    bool check = controller.signUp("piotr@sich.ua", "Пётр", "Иванович",
        "Могила", "104224", "дубашуба", "дубашуба", datastore, birthDate);
    expect(check, true);
    bool check2 = controller.signUp("piotr@sich.ua", "Пётр", "Иванович",
        "Могила", "104224", "дубашуба", "11111", datastore, birthDate);
    expect(check2, false);
    print(new DateTime.utc(1976, 3, 4, 12, 2, 2).toString().substring(0, 10));
  });
...
// final Map<String, String>
    controller.signUp("piotr@sich.ua", "Пётр", "Иванович", "Могила", "104224",
        "дубашуба", "дубашуба", datastore, birthDate);
    String wrongEmail =
        controller.login("ioann@sich.ua", "дубашуба", datastore);
    String correctPassword =
        controller.login("piotr@sich.ua", "дубашуба", datastore);
    String wrongPassword =
        controller.login("piotr@sich.ua", "шубадуба", datastore);
